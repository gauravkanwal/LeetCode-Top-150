...
## Q83. Average of Levels in Binary Tree
...
**Problem Link:** 
[Add Binary](https://leetcode.com/problems/add-binary/?envType=study-plan-v2&envId=top-interview-150)
...
**Solution Key Idea or Approach:** 
For each cell we have to return grid[i][j] + min(fn([i+1][j]), fn([i][j+1])). And then return the result of fn([0][0]) as our answer.
If we solve this recursively it would take exponential time. So We use DP here. Instead of 1D DP we need 2D DP here bcz there are two changing parameters for the recursion. As we can see there are overlapping subproblems:

### Approach 1 [Memoization]:
To improve our time complexity we use a 2d dp array and store result of each subproblem in it. This way we do not need to compute a subproblem multiple times.
TC: O(mn)
SC: O(mn + m+n) (dp array and recursion stack space)

### Approach 2 [Tabulation]:
To also save the recursive stack space we use a bottom up approach where we set the dp[m-1][n-1]=grid[m-1][n-1] get the optimal answer for each remaining cell. With the formula dp[i][j]=grid[i][j] + min(dp[i+1][j],dp[i][j+1]) . Then we return dp[0][0] which stores the min path sum.
TC: O(m*n) and SC: O(m*n)(dp array only)
